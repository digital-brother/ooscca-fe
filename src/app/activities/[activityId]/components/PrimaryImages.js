...
import { getPrimaryImages, patchImage, deleteImage, postImage } from "@/app/activities/apiImage.mjs";
...
export default function PrimaryImages() {
  ...
  async function deleteMarkedForDeletingImages() {
    ...
    setFilesData((filesData) => filesData.filter((f) => f.id || (!f.id && !f?.markedForDeleting)));
